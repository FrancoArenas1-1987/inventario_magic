...
# Carpeta donde están este script, auto_etiquetar_renombrar.py e inventario
PROJECT_DIR = Path(r"C:\Users\franc\OneDrive\Magic\inventario_magic")

# Carpeta con las imágenes procesadas (las que genera auto_etiquetar_renombrar.py)
PROCESADAS_DIR = Path(r"C:\Users\franc\OneDrive\Magic\MagicCards\Procesadas")

# Carpeta que ES tu repo de GitHub Pages (clonado con git clone)
DEPLOY_DIR = Path(r"C:\Users\franc\OneDrive\Magic\tienda_web")

# Nombre del CSV de inventario (lo genera construir_inventario_desde_fotos.py)
INVENTORY_CSV = PROJECT_DIR / "inventario_cartas.csv"

# Archivo HTML final que va a servir GitHub Pages
OUTPUT_HTML = DEPLOY_DIR / "index.html"

# Carpeta de imágenes dentro de la web (junto a index.html)
DEPLOY_IMAGES_DIR = DEPLOY_DIR / "images"

# Extensiones de imagen que consideramos
...
def run_cmd(cmd, cwd: Path) -> int:
    """Ejecuta un comando en subprocess y muestra salida básica."""
    print(f"\n[CMD] ({cwd}) -> {' '.join(cmd)}")
    result = subprocess.run(cmd, cwd=str(cwd), text=True)
    if result.returncode != 0:
        print(f"[ERROR] Comando terminó con código {result.returncode}.")
...
def format_clp(value: str) -> str:
    """Formatea CLP con puntos de miles."""
    if not value:
        return "Consultar"
    try:
        n = int(value)
...
def load_inventory(path: Path):
    """Carga inventario y filtra cartas disponibles."""
    if not path.exists():
        raise SystemExit(f"[ERROR] No se encontró el archivo de inventario: {path}")
...
def build_card_html(row: dict) -> str:
    ...
    if is_foil:
        tags_parts.append("FOIL")
    if fmt:
        tags_parts.append(fmt)
    tags_text = " • ".join(tags_parts)

    foil_badge = "<span class='foil-badge'>FOIL</span>" if is_foil else ""
...
def build_full_html(cards_html: str) -> str:
    """HTML completo de la tienda (diseño con buscador + filtro)."""
  ...
  <style>
    ...
    * {{
    ...
    .foil-badge {{
      ...
      font-size: 0.75rem;
      border-radius: 999px;
      background: linear-gradient(135deg, #ffd86b, #fc5cff);
      color: #15151f;
      font-weight: 700;
      box-shadow: 0 0 8px rgba(252, 92, 255, 0.6);
    }}
...
def copy_images():
    ...
    if not PROCESADAS_DIR.exists():
        raise SystemExit(f"[ERROR] No existe la carpeta de procesadas: {PROCESADAS_DIR}")
...
def git_commit_and_push():
    """Hace git add, commit y push en DEPLOY_DIR."""
    if not (DEPLOY_DIR / ".git").exists():
        print(f"[WARN] {DEPLOY_DIR} no es un repositorio git (.git no existe).")
        print("Asegúrate de haber hecho 'git clone' en esa carpeta.")
        return
    ...
    if not changes:
        print("[INFO] No hay cambios para commitear. Saltando git commit/push.")
        return

    # git add .
    if run_cmd(["git", "add", "."], DEPLOY_DIR) != 0:
        print("[ERROR] git add falló. Revisa el repo.")
        return

    # git commit
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    msg = f"Actualizar tienda {now}"
    if run_cmd(["git", "commit", "-m", msg], DEPLOY_DIR) != 0:
        print("[WARN] git commit falló (quizás no hay cambios nuevos).")
        return

    # git push
    if run_cmd(["git", "push"], DEPLOY_DIR) != 0:
        print("[ERROR] git push falló. Revisa credenciales o remoto.")
        return

    print("[OK] Cambios enviados a GitHub. GitHub Pages se actualizará solo.")


# ========== PROCESO PRINCIPAL ==========

def main():
    print("========== ACTUALIZANDO TIENDA MAGIC ==========")

    # 1) Ejecutar scripts de procesamiento
    ...
    if not rows:
        raise SystemExit("[ERROR] No hay cartas disponibles en el inventario.")

    # 3) Construir HTML de las tarjetas
    cards_html = "\n".join(build_card_html(r) for r in rows)

    # 4) Crear carpeta de deploy y escribir index.html
    DEPLOY_DIR.mkdir(parents=True, exist_ok=True)
    full_html = build_full_html(cards_html)
